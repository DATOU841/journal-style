...
正式任务必须声明或解析为 `run_mode=full`。题录/摘要层输出是 full-depth 链路中的中间证据，不能作为完成态；历史 `light` / `standard` 字段必须被视为旧口径并按 `full` 路径继续验收。`METADATA_ONLY_NOT_FULLTEXT_READY` 只能表示阻塞，不表示交付成功。
...
- `025-rag-import/zotero-pdf-rag-handoff-input.json`
